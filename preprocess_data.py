import torch
from torch.utils.data import Dataset
import numpy as np

CHARISOSMISET = {"#": 29, "%": 30, ")": 31, "(": 1, "+": 32, "-": 33, "/": 34, ".": 2,
                 "1": 35, "0": 3, "3": 36, "2": 4, "5": 37, "4": 5, "7": 38, "6": 6,
                 "9": 39, "8": 7, "=": 40, "A": 41, "@": 8, "C": 42, "B": 9, "E": 43,
                 "D": 10, "G": 44, "F": 11, "I": 45, "H": 12, "K": 46, "M": 47, "L": 13,
                 "O": 48, "N": 14, "P": 15, "S": 49, "R": 16, "U": 50, "T": 17, "W": 51,
                 "V": 18, "Y": 52, "[": 53, "Z": 19, "]": 54, "\\": 20, "a": 55, "c": 56,
                 "b": 21, "e": 57, "d": 22, "g": 58, "f": 23, "i": 59, "h": 24, "m": 60,
                 "l": 25, "o": 61, "n": 26, "s": 62, "r": 27, "u": 63, "t": 28, "y": 64}
CHARISOSMILEN = 64


CHARPROTSET = {"A": 1, "C": 2, "B": 3, "E": 4, "D": 5, "G": 6,
               "F": 7, "I": 8, "H": 9, "K": 10, "M": 11, "L": 12,
               "O": 13, "N": 14, "Q": 15, "P": 16, "S": 17, "R": 18,
               "U": 19, "T": 20, "W": 21, "V": 22, "Y": 23, "X": 24, "Z": 25}
CHARPROTLEN = 25


def label_smiles(line, smi_ch_ind, MAX_SMI_LEN=100):
    X = np.zeros(MAX_SMI_LEN,dtype=np.int64())
    for i, ch in enumerate(line[:MAX_SMI_LEN]):
        X[i] = smi_ch_ind.get(ch, 0)
    return X


def label_sequence(line, smi_ch_ind, MAX_SEQ_LEN=1000):
    X = np.zeros(MAX_SEQ_LEN,np.int64())
    for i, ch in enumerate(line[:MAX_SEQ_LEN]):
        X[i] = smi_ch_ind[ch]
    return X

"""
    标签One-hot编码
"""
def get_labels(label):
    coding = torch.zeros(2, dtype=torch.float)
    # [1,0]表示有交互，[0,1]表示无交互
    if label == '1':
        coding[0] = np.float(1)
    elif label == '0':
        coding[1] = np.float(1)
    return coding


"""
    加载批量数据（将样本列表转换为批次数据的形式）
"""
def collate_fn(batch_data,max_d=100,max_p=1000):
    N = len(batch_data)
    compound_new = torch.zeros((N, max_d), dtype=torch.long)
    protein_new = torch.zeros((N, max_p), dtype=torch.long)
    #labels_new = torch.zeros((N, 2), dtype=torch.float)
    labels_new = torch.zeros(N)
    for i,data in enumerate(batch_data):
        pair, label = data
        compoundstr, proteinstr = pair
        compoundint = torch.from_numpy(label_smiles(compoundstr, CHARISOSMISET,max_d))
        compound_new[i] = compoundint
        proteinint = torch.from_numpy(label_sequence(proteinstr, CHARPROTSET,max_p))
        protein_new[i] = proteinint
        #labels_new[i] = get_labels(label)
        labels_new[i] = int(label)
    return (compound_new, protein_new, labels_new)


def new_collate_fn(batch_data,max_d=100,max_p=1000):
    N = len(batch_data)
    compound_new = torch.zeros((N, max_d), dtype=torch.long)
    protein_new = torch.zeros((N, max_p), dtype=torch.long)
    labels_new = torch.zeros((N, 2), dtype=torch.float)
    for i,data in enumerate(batch_data):
        DTI, pred = data
        pair, label= DTI
        compoundstr, proteinstr= pair
        compoundint = torch.from_numpy(label_smiles(compoundstr, CHARISOSMISET,max_d))
        compound_new[i] = compoundint
        proteinint = torch.from_numpy(label_sequence(proteinstr, CHARPROTSET,max_p))
        protein_new[i] = proteinint
        labels_new[i] = pred
    return (compound_new, protein_new, labels_new)

class CustomDataSet(Dataset):
    def __init__(self, pairs):
        self.pairs = pairs

    def __getitem__(self, item):
        return self.pairs[item]

    def __len__(self):
        return len(self.pairs)
